class Item:
    ...
    def find_target(self):
        if not self.targets:
            return True
        res = next((t for t in self.targets if t.find()), None)
        if res:
...
items = [
    ...
    Item(0x4686, loc='backpack', color=2075, targets=[Item(0x19ab, range=2)], desc='shadowguard dirty'),
    Item(0x4686, loc='backpack', color=0, targets=[Item(0x1512, range=3), Item(0x151a, range=3)], desc='shadowguard pure'),
